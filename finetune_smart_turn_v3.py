...
- Whisper Base encoder (74M params) instead of Tiny (39M)
...
OUTPUT_DIR = Path("checkpoints/smart_turn_pt_v3")
...
# ---------------------------------------------------------------------------
# Model — Whisper Base (74M params, 2x bigger than Tiny)
# ---------------------------------------------------------------------------

class SmartTurnModel(nn.Module):
    """Whisper Base encoder + attention pooling + classifier."""

    def __init__(self, whisper_model: str = "openai/whisper-base"):
        super().__init__()
        ...
        hidden_size = self.encoder.config.d_model  # 512 for whisper-base
...
def train(
    ...
    # ----- Create datasets -----
    # Whisper Base uses 80 mel bins, same as Tiny
    ...
    log.info("Test results (best model from epoch %d):", checkpoint["epoch"])
    ...
    results = {
        ...
        "improvements_over_v2": [
            ...
            "whisper-base (74M) instead of whisper-tiny (39M)",
